class BankaccountsController < ApplicationController
  ...
  def index
    ...
    @spending_data = Movement.find(:all, :select => "movements.bankaccount_id,movements.movdate,(select bankaccounts.name from bankaccounts where bankaccounts.id = movements.bankaccount_id) as name,sum(movements.amount) as total", :group => "movements.bankaccount_id,movements.movdate",:conditions => ["movements.bankaccount_id in (select id from bankaccounts where bankaccounts.user_id= :id and movements.mov_type=-1 and movements.movdate BETWEEN :from  AND :to)",{:id => @current_user.id, :from => Time.now.at_beginning_of_month, :to => Time.now.end_of_month}] )
