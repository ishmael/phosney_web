class BankaccountsController < ApplicationController
  ...
  def index
    ...
    @expending_data = Movement.find(:all, :select => "movements.bankaccount_id,movements.movdate,(select bankaccounts.name from bankaccounts where bankaccounts.id = movements.bankaccount_id) as name,sum(movements.amount*movements.mov_type) as total", :group => "movements.bankaccount_id,movements.movdate",:conditions => ["movements.bankaccount_id in (select id from bankaccounts where bankaccounts.user_id= ?)",@current_user.id] )
