class BankaccountsController < ApplicationController
  ...
  def index
    ...
    @spending_data = Movement.find(:all, :select => "movements.account_id,movements.movdate,(select accounts.name from accounts where accounts.id = movements.account_id) as name,sum(movements.amount) as total", :group => "movements.account_id,movements.movdate",:conditions => ["movements.account_id in (select account_id from accounts,accounts_users where accounts_users.user_id= :id and accounts.type = :account_type and accounts_users.account_id = accounts.id)  and movements.mov_type=-1 and movements.movdate BETWEEN :from  AND :to  and ( movements.user_id = :id or (movements.private =0 and movements.user_id <> :id))",{:id => @current_user.id,:account_type => 'Bankaccount', :from => Time.now.at_beginning_of_month, :to => Time.now.end_of_month}] )
